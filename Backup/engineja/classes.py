...
class Player:
    ...
    def __init__(self, hp, stamina, power, hungry, food_db, p_weapon, p_shield, defense, current_enemy=None, money=0):
        self.hp = hp
    ...
    def eat(self, food, food_list_name, food_list, player_max, food_count):
        ...
        food_list_name.remove(food.name)
        food_count -= 1
    ...
    def equip(self, new_weapon, wep_name_list, wep_list):
        if new_weapon.w_type == "shield":
            wep_list.remove(new_weapon)
            wep_name_list.remove(new_weapon.name)
            self.defense -= self.defense
            self.p_shield = new_weapon
        ...
        Player.equipped.append(new_weapon)
        Player.name_equip.append(new_weapon.name)
    ...
    def deequip(self, wep_name_list, wep_list, new_weapon, shield_list, shield_list_name):
        ...
        Player.equipped.remove(new_weapon)
        Player.name_equip.remove(new_weapon.name)
